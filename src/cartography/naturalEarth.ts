...
function ringIntersectsBounds(points: GeoPoint[], bounds: GeoBounds): boolean {
  ...
  points.forEach((point)=>{minLat=Math.min(minLat,point.lat);maxLat=Math.max(maxLat,point.lat);minLon=Math.min(minLon,point.lon);maxLon=Math.max(maxLon,point.lon);});
...
function geometryRings(geometry?: Geometry | null): GeoPoint[][] {
  ...
  if (geometry.type==='Polygon') return (geometry.coordinates as unknown[]).map(ringToPoints).filter((ring)=>ring.length>2);
  if (geometry.type==='MultiPolygon') return (geometry.coordinates as unknown[]).flatMap((polygon)=>Array.isArray(polygon)?polygon.map(ringToPoints).filter((ring)=>ring.length>2):[]);
  return [];
}
...
async function fetchCollection(resolution: NaturalEarthResolution): Promise<FeatureCollection> {
  ...
  if(!response.ok) throw new Error(`Natural Earth ${resolution} fehlt (${response.status}). Bitte node scripts/fetch-natural-earth.mjs ausführen.`);
  const value=await response.json() as FeatureCollection;
  if(value.type!=='FeatureCollection'||!Array.isArray(value.features)) throw new Error(`Natural Earth ${resolution} ist kein gültiges GeoJSON FeatureCollection.`);
  return value;
}

export async function buildGeographicScene(bounds: GeoBounds, journeyLines: GeoPoint[][], width=960, height=1320): Promise<GeographicScene> {
  ...
  const landPaths=collection.features.flatMap((feature)=>geometryRings(feature.geometry)).filter((ring)=>ringIntersectsBounds(ring,padded)).map((ring)=>`${pointsToPath(ring,project)} Z`).filter(Boolean);
  const journeyPaths=journeyLines.filter((line)=>line.length>1).map((line)=>pointsToPath(line,project)).filter(Boolean);
