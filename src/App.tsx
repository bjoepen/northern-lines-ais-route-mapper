...
export default function App() {
  ...
  useEffect(() => {
    if (isPlaying && voyage.playbackAvailable !== false) {
  ...
  }, [isPlaying, playbackSpeed, voyage.points.length, voyage.playbackAvailable]);
  ...
  return <div className="nl-app w-screen h-screen flex flex-col overflow-hidden select-none">
    ...
    <footer className="nl-chrome h-7 border-t px-4 flex items-center justify-between text-[10px] text-[#66716d] shrink-0 tracking-[0.01em]">
      <div className="flex items-center gap-3"><span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-[#6f7c68]"/>Track geladen</span><span className="text-[#c2bbad]">·</span><span>{voyage.points.length} Wegpunkte</span><span className="text-[#c2bbad]">·</span><span>{segmentCount} Segmente</span>{gapCount>0&&<><span className="text-[#c2bbad]">·</span><span>{gapCount} Gaps</span></>}<span className="text-[#c2bbad]">·</span><span>{voyage.totalDistanceNM} sm</span></div>
      <div className="flex items-center gap-3"><span className="uppercase tracking-[0.16em] text-[#6f7c68]">Northern Lines</span><span className="text-[#c2bbad]">·</span><span>{voyage.geometryOnly?'Geometry-only':voyage.northernLinesSource?'Normalized Track':voyage.metadata.mmsi ? `MMSI ${voyage.metadata.mmsi}` : 'GPS Track'}</span></div>
    </footer>
  </div>;
}
